...
import type {
  InferenceProvider as IInferenceProvider,
  InferenceRequest,
  InferenceResponse,
  Model,
  InferenceMessage,
...
import { OpenRouterApiProvider } from '@/lib/providers/openrouter/api-provider';
import { OpenRouterOAuthProvider } from '@/lib/providers/openrouter/oauth-provider';

interface InferenceContextValue {
  // Current provider state
  provider: IInferenceProvider | null;
  isLoading: boolean;
  error: string | null;

  // Provider actions
  setProvider: (provider: IInferenceProvider) => void;
  clearProvider: () => void;
...
export function InferenceProvider({ children }: InferenceProviderProps) {
  const [provider, setProviderState] = useState<IInferenceProvider | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedModelId, setSelectedModelId] = useState<string | undefined>(() => {
    // Load saved model selection from localStorage (only on client-side)
  ...
  const setProvider = useCallback((newProvider: IInferenceProvider) => {
  ...
  const restoreProviderWithModels = useCallback(async (restoredProvider: IInferenceProvider) => {
    ...
    try {
      ...
      const savedModelId = typeof window !== 'undefined' ? localStorage.getItem('selected_model_id') : null;
      ...
      if (savedModelId && restoredProvider.models.find(m => m.id === savedModelId)) {
        ...
        if (savedModelId && typeof window !== 'undefined') {
          localStorage.removeItem('selected_model_id');
        }
  ...
  const loadModels = useCallback(async (): Promise<Model[]> => {
    ...
    try {
      ...
      if (savedModelId && models.find(m => m.id === savedModelId)) {
        ...
        if (savedModelId && !models.find(m => m.id === savedModelId) && typeof window !== 'undefined') {
