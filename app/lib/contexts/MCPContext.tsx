...
import { availableServers } from '@/lib/mcp/servers';
...
export function MCPProvider({ children }: MCPProviderProps) {
  ...
  useEffect(() => {
    // Prevent loading if we've already loaded or if we're on server-side
    if (hasLoadedPersisted.current || typeof window === 'undefined') {
      return;
    }
